...
public class ComputerDAO implements DAO<Computer> {
    ...
    private static final String FIND_QUERY = "SELECT * FROM " + COMPUTER_TABLE + "  LEFT JOIN " + COMPANY_TABLE
            + " ON computer.company_id = company.id WHERE computer.id = ?;";
    private static final String FIND_ALL_QUERY = "SELECT * FROM " + COMPUTER_TABLE + " LEFT JOIN " + COMPANY_TABLE
    ...
    public long add(Computer computer) throws DaoException, DatabaseException {
        try (Connection connection = Database.getConnection();
                PreparedStatement stmt = connection.prepareStatement(ADD_QUERY, Statement.RETURN_GENERATED_KEYS);) {
            long id = -1;
