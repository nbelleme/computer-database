...
public class AddComputer extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request,
        ...
        try {
            ArrayList<Company> companies = (ArrayList<Company>) companyService
                    .findAll();
            request.setAttribute("companies", companies);
            request.getRequestDispatcher("/views/addComputer.jsp")
                    .forward(request, response);
        } catch (DaoException e) {
            request.getRequestDispatcher("/views/500.html").forward(request,
                    response);
        }
    }
    ...
    protected void doPost(HttpServletRequest request,
        ...
        Computer computer = new Computer.Builder().name(name)
                .introduced(introducedDateTime)
                .discontinued(discontinuedDateTime).company(company).build();

        if (ComputerValidator.getInstance().isValid(computer)) {
            try {
